...
def get_provider_groups():
    return pl.scan_parquet('data/claims_clean.parquet').select(c.group_name).filter(c.group_name.is_not_null()).unique().sort('group_name').collect().to_series().to_list()
...
def generate_data_by_period(product, interval,filter_groups):
    data = pl.scan_parquet('data/claims_clean.parquet').filter(c.dos.dt.date() > 2022).filter(c.pbm_id == 1)
    return(
    data
    .filter(c.fee < 10)
...
# def run(intervals: list = ('1mo','1q','1y','1w')):
#     pl.concat([generate_data_by_period(i) for i in intervals]).collect().write_parquet(DATA)
...
def create_figure(product,interval,filter_groups, qty):
    ...
    fig.update_yaxes(
        ticksuffix = '      ',
        tickfont=dict(
            family="Arial Black",  # Choose a bold font family
            size=12,
            color="grey"
    ))
    fig.update_xaxes(
...
def get_product_list() -> list:
    return pl.scan_parquet('data/claims_clean.parquet').select(c.product).unique().sort('product').collect().to_series().to_list()
